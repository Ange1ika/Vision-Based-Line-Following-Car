...
#     print("✓ dashboard.png сохранён")
    ...
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('Summary Dashboard', fontsize=16, fontweight='bold', y=0.995)
    
    # 9.1 Ошибка
    
    axes[0, 0].plot(df['frame'], df['E_raw_cm'], 'r-', alpha=0.3, linewidth=1, label='Raw')
    axes[0, 0].plot(df['frame'], roll_fil, 'b-', linewidth=2, label='Filtered')
    ...
    ax2.set_ylabel('Angle Fitline (deg)', color='orange')
    ax1.tick_params(axis='y', labelcolor='g')
    ax2.tick_params(axis='y', labelcolor='orange')
    ...
    print(f"Средний FPS: {df['fps'].mean():.1f}")
    print("=========================\n")
